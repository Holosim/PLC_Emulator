...
| CORE-208 | Math instructions (`ADD`, `SUB`, `MUL`, `DIV`) compute a result from two operands (tag or literal, `DINT`/`REAL` may mix per the CORE-207 note above) and write it to a destination tag; `DIV` by zero is a defined runtime error, not a crash. | SN-1 | Test | Approved | |
