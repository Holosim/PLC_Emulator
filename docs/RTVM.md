...
| DATA-OUT-301 | Internal runtime state serializes to the TCP/JSON output message format (current I/O tag values) for transmission to the connected simulation client. | SN-1, SN-3 | Test | Verified | |
...
| NFR-502 | Third-party dependencies are avoided by default; any dependency adopted is referenced only from behind an internal interface/wrapper, never directly from core logic, so it can be swapped later. | SN-5 | Inspection | Verified | |
