...
| OUT-400 | Server exposes a TCP listener on an operator-configurable port implementing a custom JSON protocol; exactly one external simulation client may be connected and read current tag state in real time (v1.0 single-client constraint). | SN-1, SN-3 | Test | Verified | |
