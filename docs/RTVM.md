...
- **DELIV-901 verified (issue #29, 2026-08-17), after OUT-403 (#30)
  merged.** Software Engineer re-verified TP-901 step 4 against a live
  process on the rebased branch — the `tag_write` is now genuinely
  live-observable (free-running scan loop applies it within a fraction
  of a millisecond), with one documentation wrinkle caught in the
  process: a naive single-`readline()` client can appear stuck behind
  thousands of already-buffered `tag_update` broadcasts from the
  unthrottled loop, so the guide's example client now reads in a loop
  until it observes the expected value rather than trusting the very
  next line. Test Engineer independently re-ran all five TP-901 steps
  (build, config copy, launch, live TCP exchange, and a scratch
