...
| CORE-208 | Math instructions (`ADD`, `SUB`, `MUL`, `DIV`) compute a result from two operands (tag or literal) and write it to a destination tag; `DIV` by zero is a defined runtime error, not a crash. | SN-1 | Test | Verified | `10c9dad` |
