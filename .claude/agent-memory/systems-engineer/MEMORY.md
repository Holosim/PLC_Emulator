...
- PLC_Emulator: `docs/RTVM.md`/`docs/SDD.md`/`docs/IMPLEMENTATION_PLAN.md` all populated (issues #2-#4 closed). Generate Code Base done (#5). All of #6-#26 closed, 26/27 RTVM rows `Verified`. **#27 DELIV-900 reopened 2026-08-17 post-release**: client field defect (`global.json` SDK-pin, see [[issue-closeout-log-plc-emulator]] bottom entry) — RTVM flipped back `Verified`→`In Implementation`, handed to `agent:software-engineer` with the exact one-line fix (`rollForward`→`latestMajor`). First release `v1.0.2` already shipped and stands; this is a post-release patch cycle, not a re-do of the whole project. See [[issue-closeout-log-plc-emulator]] for per-issue commit SHAs, merge-conflict notes, and open caveats (incl. #21's flagged-but-unowned scan-cadence gap).
