...
- CORE-205/CORE-206 (issue #12, `Ctu`/`Ctd`/`Res`): marked `Verified` in RTVM 2026-08-17 (fast-path `status:ready-for-rtvm-update` after Test Engineer TP-205/TP-206 pass on branch `issue-12`), handed to CI/CD; commit SHA pending. `CounterState` gained `Cu`/`Cd` edge-memory fields, signed off into SDD Tag data model (see sdd_decisions memory) — same pattern as the issue #9 `rungState` sign-off. **Git gotcha:** on this branch, `git merge origin/main` failed with "refusing to merge unrelated histories" — per-issue feature branches don't share commit ancestry with `main` in this repo's pipeline. Don't try to merge/rebase a feature branch against `main` to "catch up" docs before editing (including this MEMORY.md, which was several commits stale vs. `main` when this note was written) — just edit the doc as it exists on the feature branch and let CI/CD resolve any conflict on merge, as it already has for issues #10/#11.
