...
public sealed class XicXioOteTests
{
    ...
    public void Xic_Evaluate_MatchesTagValue(bool tagValue, bool expected)
    {
        ...
        Assert.AreEqual(expected, xic.Evaluate(tags, rungState: true, elapsed: TimeSpan.Zero));
    }
    ...
    public void Xio_Evaluate_IsNegationOfTagValue(bool tagValue, bool expected)
    {
        ...
        Assert.AreEqual(expected, xio.Evaluate(tags, rungState: true, elapsed: TimeSpan.Zero));
    }
    ...
    public void XicAndXio_Evaluate_DeEnergizedRungStateStaysFalse()
    {
        ...
        Assert.IsFalse(new Xic("C").Evaluate(tags, rungState: false, elapsed: TimeSpan.Zero));
        Assert.IsFalse(new Xio("C").Evaluate(tags, rungState: false, elapsed: TimeSpan.Zero));
    }
    ...
    public void Ote_Evaluate_WritesIncomingRungStateToTag_NonLatching()
    {
        ...
        Assert.IsTrue(ote.Evaluate(tags, rungState: true, elapsed: TimeSpan.Zero));
        Assert.AreEqual(true, tags.Get("B").Value);

        Assert.IsFalse(ote.Evaluate(tags, rungState: false, elapsed: TimeSpan.Zero));
    ...
    public void InstructionFactory_BuildsRealXicXioOte_ThatEvaluateCorrectly()
    {
        ...
        Assert.IsTrue(xic.Evaluate(tags, rungState: true, elapsed: TimeSpan.Zero));
        Assert.IsFalse(xio.Evaluate(tags, rungState: true, elapsed: TimeSpan.Zero));
        Assert.IsTrue(ote.Evaluate(tags, rungState: true, elapsed: TimeSpan.Zero));
        Assert.AreEqual(true, tags.Get("B").Value);
    }
}
